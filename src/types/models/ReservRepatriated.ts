...
export class ReservRepatriated implements Entity {
    ...
    static async get(id:string): Promise<ReservRepatriated | undefined>{
        ...
        if (record){
            return ReservRepatriated.create(record);
        }else{
            return;
        }
    }
    ...
    static create(record: Partial<Omit<ReservRepatriated, FunctionPropertyNames<ReservRepatriated>>> & Entity): ReservRepatriated {
